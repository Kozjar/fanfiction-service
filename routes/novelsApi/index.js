...
const {checkIsLoggedIn, checkIsLoggedInStrict} = require('../loginCheck')
...
router.get('/:id', checkIsLoggedIn, (req, res) => {
  ...
  .then(novel => {
    ...
    const resObj = {
      ...tmpNovel._doc,
      userRate: novel.user_rate.id(req.session.userId) || {rate: 0},
      accessStatus: status,
      commentsCount: tmpNovel.comments.length,
      rate_count: tmpNovel.user_rate.length,
    }
    delete resObj.comments;
    console.log(resObj);
    res.send(resObj)
  })
  .catch(err => {
